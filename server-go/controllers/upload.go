...
func (uc *UploadController) UploadImage(c *fiber.Ctx) error {
	...
	if uc.State.Config.CloudinaryConfigured() {
		...
		client, err := services.NewCloudinaryClient(uc.State.Config)
		if err == nil {
	...
	if err := os.MkdirAll(uc.UploadsDir, 0o755); err != nil {
	...
	savePath := filepath.Join(uc.UploadsDir, filename)
...
func (uc *UploadController) DeleteImage(c *fiber.Ctx) error {
	_, ok := middleware.CurrentUser(c)
	if !ok {
		return respondError(c, fiber.StatusUnauthorized, "Unauthorized")
	}
	...
	if filename != "" {
		_ = os.Remove(filepath.Join(uc.UploadsDir, filename))
	}
