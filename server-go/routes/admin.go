...
func RegisterAdmin(router fiber.Router, state *app.State) {
	router.Get("/admin/users", middleware.RequireAuth(state.Config), func(c *fiber.Ctx) error {
		...
		cursor, err := state.Mongo.Users().Find(ctx, bson.M{})
		if err != nil {
	...
	router.Post("/admin/users/:id/block", middleware.RequireAuth(state.Config), func(c *fiber.Ctx) error {
		userCtx, ok := middleware.CurrentUser(c)
		if !ok || userCtx.Role != "super-admin" {
			return respondError(c, fiber.StatusForbidden, "Admin access required")
		}
		...
		err = state.Mongo.Users().FindOneAndUpdate(
			ctx,
			bson.M{"_id": objID},
