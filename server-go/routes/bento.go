...
func RegisterBento(router fiber.Router, state *app.State) {
	router.Get("/bento/:username", func(c *fiber.Ctx) error {
		...
		err := state.Mongo.Users().FindOne(ctx, bson.M{"username": username}).Decode(&user)
		...
		err = state.Mongo.BentoConfigs().FindOne(ctx, bson.M{"username": username}).Decode(&config)
		if err == mongo.ErrNoDocuments {
		...
		if state.Redis != nil {
			_ = state.Redis.SetJSON(ctx, cacheKey, &config, 60*time.Second)
		}

		return c.JSON(config)
	})

	router.Post("/bento/sync", middleware.RequireAuth(state.Config), func(c *fiber.Ctx) error {
		...
		err := state.Mongo.Users().FindOne(ctx, bson.M{"_id": userCtx.ID}).Decode(&dbUser)
		if err == mongo.ErrNoDocuments {
			return respondError(c, fiber.StatusNotFound, "User not found")
		}
		if err != nil {
		...
		err = state.Mongo.BentoConfigs().FindOneAndUpdate(ctx, bson.M{"user": userCtx.ID}, update, opts).Decode(&updated)
		if err != nil {
			return respondError(c, fiber.StatusInternalServerError, "Sync failed")
		}

		if state.Redis != nil {
			_ = state.Redis.Del(ctx, fmt.Sprintf("bento:%s", dbUser.Username))
		}
